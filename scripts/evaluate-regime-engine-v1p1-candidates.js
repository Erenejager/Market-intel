#!/usr/bin/env node
/*
  Regime engine v1.1 candidate evaluator.
...
const OUT_JSON = path.join(DATA, 'regime-engine-v1p1-candidate-evaluation.json');
const OUT_MD = path.join(DATA, 'regime-engine-v1p1-candidate-evaluation.md');
...
const flushRules = [];
// Keep v1.1 tractable and interpretable: a small mechanism grid around the best v1 near-misses.
...
const result = {
  ...
  inputs: { prices: PRICE_PATH, labels: LABELS_PATH, plan: path.join(DATA, 'regime-engine-v1p1-plan.md'), preregistration: path.join(DATA, 'regime-engine-v1-preregistration.md') },
  sample_counts: {
...
function renderMd() {
  const lines = [];
  lines.push('# Regime Engine v1.1 Candidate Evaluation', '', `Generated: ${result.generated_at}`, '', '## Scope', '', '- Diagnostic only.', '- Adds hysteresis and FLUSH sub-state per `data/regime-engine-v1p1-plan.md`.', '- Reuses frozen preregistration criteria; no alert wiring.', '', '## Pass counts', '', '```json', JSON.stringify(passCounts, null, 2), '```', '', '## Top candidates', '', '| squeeze rule | bearish rule | flush rule | sq e/x | bear e/x | flush ttl | all | sq | bear | flush | global | flips/day | flickers | sq recall | bear cov | bear FPR | flush cov |', '| --- | --- | --- | ---: | ---: | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |');
