#!/usr/bin/env node
/*
  Regime engine v1.2 candidate evaluator.
...
const OUT_JSON = path.join(DATA, 'regime-engine-v1p2-candidate-evaluation.json');
const OUT_MD = path.join(DATA, 'regime-engine-v1p2-candidate-evaluation.md');
...
const bearRules = [
  { name: 'dist5d_lt_-1', pred: r => r.features.dist5d < -1 },
  { name: 'dist5d_lt_-0.5', pred: r => r.features.dist5d < -0.5 },
  { name: 'ret7d_lt_-1', pred: r => r.features.ret7d < -1 },
];
const flushRules = [];
// Keep v1.2 tractable and interpretable: a small mechanism grid around the best v1 near-misses.
for (const ret4 of [-0.75, -1, -1.25, -1.5]) {
  for (const accel of [-0.25, -0.5]) {
    for (const ratio of [1.5, 1.75]) {
      flushRules.push({ name: `ret4h_lt_${ret4}_accel_lt_${accel}_range_ratio_gt_${ratio}`, pred: r => r.features.ret4h < ret4 && r.features.accel4h < accel && r.features.range4h_ratio > ratio });
    }
  }
}
for (const ret4 of [-0.75, -1, -1.25, -1.5]) {
  for (const dist of [-0.5, -1, -1.5]) {
    for (const ratio of [1.5, 1.75]) {
      flushRules.push({ name: `ret4h_lt_${ret4}_dist5d_lt_${dist}_range_ratio_gt_${ratio}`, pred: r => r.features.ret4h < ret4 && r.features.dist5d < dist && r.features.range4h_ratio > ratio });
    }
  }
}
...
function simulate({ squeezeRule, bearRule, flushRule, squeezeEntry, squeezeExit, bearEntry, bearExit, flushEntry = 3, flushExit = 3, flushTtlHours }) {
  ...
  for (const r of samples) {
    updateHyst(sq, !!squeezeRule.pred(r), r, squeezeEntry, squeezeExit);
    updateHyst(br, !!bearRule.pred(r), r, bearEntry, bearExit);

    // v1.2 squeeze-exit override:
    ...
    out.push({ ...r, state, raw: { squeeze: !!squeezeRule.pred(r), bearish: !!bearRule.pred(r), flush: !!flushRule.pred(r) } });
  }
  return out;
}
  ...
  for (const bearRule of bearRules) {
    for (const flushRule of flushRules) {
      for (const squeezeEntry of [2, 3]) for (const squeezeExit of [4, 6]) {
        for (const bearEntry of [3, 4]) for (const bearExit of [4, 6, 8]) {
          for (const flushTtlHours of [8, 12]) {
            const cfg = { squeezeRule, bearRule, flushRule, squeezeEntry, squeezeExit, bearEntry, bearExit, flushTtlHours };
            const rows = simulate(cfg);
            const sc = score(rows);
            candidates.push({
              config: { squeeze: squeezeRule.name, bearish: bearRule.name, flush: flushRule.name, squeezeEntry, squeezeExit, bearEntry, bearExit, flushEntry: 3, flushExit: 3, flushTtlHours },
              score: sc,
            });
          }
        }
...
const result = {
  ...
  inputs: { prices: PRICE_PATH, labels: LABELS_PATH, plan: path.join(DATA, 'regime-engine-v1p2-plan.md'), preregistration: path.join(DATA, 'regime-engine-v1-preregistration.md') },
  sample_counts: {
...
function line(c) {
  const s = c.score;
  return `| ${c.config.squeeze} | ${c.config.bearish} | ${c.config.flush} | ${c.config.squeezeEntry}/${c.config.squeezeExit} | ${c.config.bearEntry}/${c.config.bearExit} | ${c.config.flushTtlHours}h | ${s.all_pass ? 'PASS' : 'FAIL'} | ${s.squeeze.pass ? 'Y' : 'N'} | ${s.bearish.pass ? 'Y' : 'N'} | ${s.flush.pass ? 'Y' : 'N'} | ${s.global.pass ? 'Y' : 'N'} | ${s.global.flips_per_day.toFixed(2)} | ${s.global.sub45_flickers} | ${pct(s.squeeze.recall)} | ${pct(s.bearish.coverage)} | ${pct(s.bearish.false_positive_rate)} | ${pct(s.flush.coverage)} |`;
}
function renderMd() {
  const lines = [];
  lines.push('# Regime Engine v1.2 Candidate Evaluation', '', `Generated: ${result.generated_at}`, '', '## Scope', '', '- Diagnostic only.', '- Adds hysteresis, FLUSH sub-state, and bearish-confirmed squeeze-exit override.', '- Reuses frozen preregistration criteria; no alert wiring.', '', '## v1.2 change', '', 'If BEARISH_TREND is established, stale BULLISH_SQUEEZE state is forcibly cleared. This targets the v1.1 failure where high-recall squeeze rules persisted too long inside bearish windows.', '', '## Pass counts', '', '```json', JSON.stringify(passCounts, null, 2), '```', '', '## Top candidates', '', '| squeeze rule | bearish rule | flush rule | sq e/x | bear e/x | flush ttl | all | sq | bear | flush | global | flips/day | flickers | sq recall | bear cov | bear FPR | flush cov |', '| --- | --- | --- | ---: | ---: | ---: | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |');
  ...
  else lines.push('- No candidate passed all preregistered historical criteria. Do not wire shadow-live classifier yet without a v1.2 plan.');
